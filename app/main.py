...
from sqlalchemy import Column, DateTime, Integer, Text, create_engine, func, text
...
async def get_generation_result(task_id: str):
    ...
    if task_result["status"] == "processing":
        ...
        # In a production use a proper cache with expiration)
        return {
            "status": "completed",
...
async def create_embedding(text_input: TextInput, db: Session = Depends(get_db)):
    try:
        ...
        return {
            ...
            "created_at": str(db_embedding.created_at),
        }
    except Exception as e:
